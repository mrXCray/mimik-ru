...
| BookStack | `core/export/bookstack-export.ts` | BookStack Portable ZIP (v24.12+ import): `data.json` page with Markdown, screenshots in `files/`, linked as `[[bsexport:image:N]]` |
...
- **AI limits** (`core/capture/ai/limits.ts`, per profile): max answer tokens, request timeout and the post-recording wait for step descriptions; 0 means unlimited and is the token default. With "use my own server" on, AI works without an API key (`resolveAiKey().usable`; `NO_KEY_PLACEHOLDER` is sent). Settings' **Test** button runs `testAiModel` (a real step-description request with the unsaved settings) via the `testAi` message
- **Built-in step rules** (`STEP_STYLE_RULES` + per-language notes in `prompts.ts`): appended to step-description prompts only (`applyPromptSettings(…, { stepRules: true })`), after the task and before the language rule; per-profile `aiStyleGuide`, on by default; project context overrides them. With rules on, `tidyStepDescription` strips a copied `(click)` marker and a closing period. Examples inside the rules use NAME/VALUE placeholders: small models copy literal example labels
- **Editor AI** (`core/capture/ai/{guide-ai,proofread,step-note,step-context}.ts`, `entrypoints/background/guide-text.ts`): every step keeps its serialized `domContext` from capture, and `describeStepContext` (with an `elementMeta` fallback for older steps) sends that screen context with proofreading and note requests. **Proofread** (`ProofreadDialog`) checks title, description, steps, blocks and notes one request at a time, shows a word diff and applies the chosen fixes after a snapshot; `isPlausibleCorrection` drops answers that echo the prompt, add lines, change the word count by more than ~20% or keep under 60% of the original word stems. **Step notes** (`Step.note`) are typed or AI-written under the screenshot and exported by HTML, PDF, DOCX and Markdown/BookStack
